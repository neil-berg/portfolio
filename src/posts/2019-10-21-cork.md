...
title: "Cork"
date: "2019-10-21"
description: A React + Express app to share photos of wines and explore what other are drinking too
---
...
In production, `MONGODB_URL` should reflect the URL for wherever the DB will be hosted publically. If using Mongo Atlas, creating a multi-server cluster to host the DB on the cloud will return a new URL that should be the value to `MONGODB_URL`. This environmental variable can be injected into the app via Heroku, for example, or another hosting platform for the app.
...
2. Compile the model from schema definitions and define a collection name for the model
...
router.post('/api/wines/', auth, async (req, res) => {
  // req.user is accessible via the auth middle ware
